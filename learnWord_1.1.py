...
FILE_IMPARATE = "parole_imparate.json"

# Carica parole attive
if os.path.exists(FILE_NAME):
    with open(FILE_NAME, "r") as f:
        wordsInFile = json.load(f)
else:
    wordsInFile = []

# Carica parole memorizzate
if os.path.exists(FILE_IMPARATE):
    with open(FILE_IMPARATE, "r") as f:
        wordsLearned = json.load(f)
else:
    wordsLearned = []

def salva_parole():
    with open(FILE_NAME, "w") as f:
        json.dump(wordsInFile, f, indent=4)

def salva_parole_imparate():
    with open(FILE_IMPARATE, "w") as f:
        json.dump(wordsLearned, f, indent=4)
...
def weighted_sample_no_replacement(items, k):
    """
    Seleziona k parole senza ripetizione, con probabilità	
    proporzionale a (errors + 1). Le nuove parole (errors = 0)	
    hanno un bonus moltiplicativo di 1.5 per essere più probabili.
    """
    items = items.copy()
